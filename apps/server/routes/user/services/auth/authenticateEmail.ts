   ...
   try {
      await Connection.transaction(async transaction => {
         ...
         return jwt.verify(emailToken, process.env.JWT_KEY!, async (error: any) => {
            try {
               const authentication = await Authentication.findOne({ where: { emailToken } })
               if (error || !authentication) {
                  ...
                  throw new ApiError('The link to authenticate your email address is invalid', 400)
               }
               if (authentication.authenticated) {
                  throw new ApiError('The email address provided is already authenticated', 400)
               }
